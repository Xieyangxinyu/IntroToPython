...
optimization if you use the correct data structures (dicts when you
need fast lookup by a key, lists when you need ordered iteration).
...
class RandomWriter(object):
    ...
    def generate(self):
        ...
        while True:
            ...
            if self.graph.check(word):
                if len(curState) > 0:
                    token = curState.popleft()
                if len(curState) == 0:
                    curState = self.graph.newState()
                    word = ' '.join(curState)
            else :
                curState.append(self.graph.count[word].getToken())
                token = curState.popleft()
            
            yield self.graph.original(token)
    ...
    def generate_file(self, filename, amount):
        ...
        """
        lst = list(itertools.islice(self.generate(), amount))
    ...
    def train_url(self, url):
        ...
        """
        self.train_iterable(requests.get(url).text)
    ...
    def train_iterable(self, data):
        ...
        elif self.tokenization == Tokenization.byte and not isinstance(data, bytes):
            data = data.encode('UTF-8')
        self.graph.train(data, self.level)
